...
public class HeartPathAnimator {
    ...
    public static class Config {
        ...
        static public Config fromTypeArray(TypedArray typedArray, float x, float y, int pointx, int heartWidth, int heartHeight) {
            ...
            config.initX = (int) typedArray.getDimension(R.styleable.live_heart_layout_initX,
                    x);
            config.initY = (int) typedArray.getDimension(R.styleable.live_heart_layout_initY,
                    y);
            config.xRand = (int) typedArray.getDimension(R.styleable.live_heart_layout_xRand,
                    res.getDimensionPixelOffset(R.dimen.live_heart_anim_bezier_x_rand));
            config.animLength = (int) typedArray.getDimension(R.styleable.live_heart_layout_animLength,
                    res.getDimensionPixelOffset(R.dimen.live_heart_anim_length));//动画长度
            config.animLengthRand = (int) typedArray.getDimension(R.styleable.live_heart_layout_animLengthRand,
                    res.getDimensionPixelOffset(R.dimen.live_heart_anim_length_rand));
            config.bezierFactor = typedArray.getInteger(R.styleable.live_heart_layout_bezierFactor,
...
//                    res.getDimensionPixelOffset(R.dimen.heart_size_height));//动画图片高度
            ...
            config.animDuration = typedArray.getInteger(R.styleable.live_heart_layout_anim_duration,
                    res.getInteger(R.integer.live_anim_duration));//持续期
            return config;
        }
    }
