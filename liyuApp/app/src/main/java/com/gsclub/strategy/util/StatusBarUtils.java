...
//                attributes.flags |= flagTranslucentStatus;
    ...
    /**
     * 用于设置常规页面改变状态栏颜色，使之与标题颜色一致
     * @param activity
     * @param color
     */
    public static void setNormal(Activity activity, int color) {
        with(activity).setColor(activity.getResources().getColor(color)).init();
    }
     ...
     * @param activity
     */
    public static void setImageTop(Activity activity) {
        with(activity).init();
    }
}
