...
public class CalculationUtil {
    ...
    public static String roundRuturnString(double value, int length) {
        if (length <= 0)
...
//        df.setRoundingMode(RoundingMode.FLOOR);
    ...
    public static double mul(double... doubles) {
        ...
        for (int i = 0, size = doubles.length; i < size; i++) {
    ...
    /**
     * 提供精确的小数位四舍五入处理。
     * 数值仅用于计算/显示请使用{@link CalculationUtil#roundRuturnString}
     *
     * @param v     需要四舍五入的数字
...
//        return b.divide(one, scale, BigDecimal.ROUND_HALF_UP).doubleValue();
        ...
        BigDecimal bg = new BigDecimal(v).setScale(scale, RoundingMode.HALF_DOWN);
...
//        return b.divide(one, scale, BigDecimal.ROUND_HALF_UP).doubleValue();
        ...
        BigDecimal bg = new BigDecimal(v).setScale(scale, mode);
        return bg.doubleValue();
    }
